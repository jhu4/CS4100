    ...
    break;
      ...
      PAINTSTRUCT ps;
        ...
        if (m_Buffer.size() > 1)
        {
          ...
          int FinishIndex = m_Buffer.size();
          if ( FinishIndex > StartIndex+PageSize+1 )
		  {
            FinishIndex = StartIndex+PageSize+1;
		  }

          std::vector<std::string>::iterator beg = m_Buffer.begin() + StartIndex;
          // Fixed by Rich 11/08 (assertion check failed since end out of range)
          // std::vector<std::string>::iterator end = m_Buffer.begin() + StartIndex+PageSize+1;
          std::vector<std::string>::iterator end = m_Buffer.begin() + FinishIndex;
          int line=0;
