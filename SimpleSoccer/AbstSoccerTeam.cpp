                          ...
                          Vector2D& ShotTarget)const
{
  ...
  while (NumAttempts--)
  {
    ...
    int MinYVal = static_cast<int>( OpponentsGoal()->LeftPost().y + Pitch()->Ball()->BRadius());
    int MaxYVal = static_cast<int>( OpponentsGoal()->RightPost().y - Pitch()->Ball()->BRadius());
...
void AbstSoccerTeam::Render()const
{
  ...
  if (Prm.bShowControllingTeam)
  {
    ...
    {
      gdi->TextAtPos(Pitch()->cxClient()-200, 3, "Controlling Player: " + ttos(m_pControllingPlayer->ID()));
    }
  }
